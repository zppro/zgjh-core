...
- (void)setCurrentPage:(NSInteger)pageNumber
{
    ...
    //added by zppro 增加代理方法
    if(delegate != nil && [delegate respondsToSelector:@selector(dDPageControl:currentPageChangedFrom:to:)]){
        [delegate dDPageControl:self currentPageChangedFrom:oldPage to:currentPage];
    }
}
